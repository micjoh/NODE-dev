...
-	Plugin Authors can now add their contact information and plugin help in a separate function callable from the Info button in settings (Thanks hoppfrosch)
...
MakeGUI_Conf:
	...
	Gui, Add, Button, x10 y470 w100 h30 gConfInfo, Plugin Info
	Gui, Add, Tab2, x5 y5 w750 h450 vCommon_C_Tab, Main|%Plugs_ConfGUIList%
...
ConfInfo:
	Gui, %cGUI_Conf%:+LastFound
	Gui, Submit, NoHide
	If IsFunc("P_" Common_C_Tab "_ConfInfo")
	{
		P_%Common_C_Tab%_ConfInfo(Common_C_Author, Common_C_Info)
		Msgbox, 0, %AName% - %Common_C_Tab%, Plugin Author : %Common_C_Author%`n`nPlugin Info:`n%Common_C_Info%
	}
	else
	{
		Msgbox, 0, %AName% - %Common_C_Tab%, No information added by plugin author.
	}
return

P_Main_ConfInfo(ByRef Author, ByRef Info)
{
	Author = Rajat
	Info = Just a simple plugin to configure NODE's own options. For plugin specific options and information please go to specific tabs.
}
...
LaunchHomepage:
	Run, http://%AWebsite%,, UseErrorLevel
Return
