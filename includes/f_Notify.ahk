...
;————————————————————————————————————————————————————————————————————————
 ...
 If (GR_ && !is9x)					; may have to be removed for seperate title/message/etc actions
  WinSet, Region, % "0-0 w" GW " h" GH " R" GR_ "-" GR_
 If (GT_ && !is9x)					; non-functioning GT option, since the border gui gets in the way
...
Gui, %GN%:Show,  % "Hide x" NewX " y" NewY " w" GW, _Notify()_GUI_			; actual creation of Notify() gui! but still not shown
 WinGet, i, ID, _Notify()_GUI_
If SI_
  DllCall("AnimateWindow", "UInt", i, "Int", SI_, "UInt", 0x40008)		; animated in, if SI is used
 Else
  Gui, %GN%:Show, NA, _Notify()_GUI_				; otherwise, just shown
 Gui, %GN2%:Show, NA, _Notify()_BGGUI_				; and the border shown
 WinSet, AlwaysOnTop, On, _Notify()_GUI_					; and set to Always on Top
